...
- Make a subdirectory `RawData` under `examples/datasets` if it does not exist.
- Download Tiny-ImageNet-200 from Stanford:
  `http://cs231n.stanford.edu/tiny-imagenet-200.zip`
- Extract it so that this path exists:
  `examples/datasets/RawData/tiny-imagenet-200/train`
  and `examples/datasets/RawData/tiny-imagenet-200/val`.
- Use `./resources/dataset/tiny_imagenet_dataset.py` with dataset function
  `get_tiny_imagenet` in your client config.
- You can also enable auto download in `dataset_kwargs`:
  `auto_download: True` (default) and optionally set
  `download_url` / `keep_archive`.
