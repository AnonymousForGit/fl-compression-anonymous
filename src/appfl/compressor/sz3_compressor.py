...
class SZ3Compressor(BaseCompressor):
    ...
    def _decompress_model(
        ...
        # IMPORTANT:
        # Keep output keyset identical to compressed input keyset.
        # Expanding to all `model.state_dict()` keys can introduce alias keys
        # (e.g. conv_layer vs block.0) and breaks gradient-mode aggregation.
        for name, source_bytes in compressed_weights.items():
            ref_key = self._resolve_model_key(name, model_state)
            if ref_key is None:
                # Skip unknown keys instead of fabricating alias entries.
                continue
            param = model_state[ref_key]

            if "weight" in name and param.numel() > self.param_count_threshold:
                restored = self._decompress(
                    cmp_data=source_bytes,
                    ori_shape=(param.numel(),),
                    ori_dtype=np.float32,
                ).astype(np.float32)
            else:
                if self.lossless_compressor == "zstd":
                    raw = zstd.decompress(source_bytes)
                elif self.lossless_compressor == "gzip":
                    raw = gzip.decompress(source_bytes)
                elif self.lossless_compressor == "zlib":
                    raw = zlib.decompress(source_bytes)
                elif self.lossless_compressor == "blosc":
                    raw = blosc.decompress(source_bytes, as_bytearray=True)
                elif self.lossless_compressor == "lzma":
                    raw = lzma.decompress(source_bytes)
                else:
                    raise NotImplementedError
                restored = np.frombuffer(raw, dtype=param.detach().cpu().numpy().dtype)

            copy_arr = deepcopy(restored)
            copy_tensor = torch.from_numpy(copy_arr).to(param.dtype).reshape(param.shape)
            decompressed_weights[name] = copy_tensor
        return decompressed_weights

    def _resolve_compressed_key(
        self, name: str, compressed_weights: Union[dict, OrderedDict]
    ) -> Optional[str]:
        """
        Resolve possible key mismatches between sender and receiver model naming.
        Handles common cases such as:
          - DataParallel prefix differences (`module.`)
          - U-Net alias differences (`conv_layer`/`norm_layer` vs `block.*`)
        """
        candidates = [name]
        ...
        # Common U-Net aliases observed across instantiation paths.
        alias_pairs = [
            (".block.0.", ".conv_layer."),
            (".conv_layer.", ".block.0."),
            (".block.1.", ".norm_layer."),
        ...
        for src, dst in alias_pairs:
            for c in base_candidates:
                if src in c:
                    candidates.append(c.replace(src, dst))

        for candidate in candidates:
            if candidate in compressed_weights:
                return candidate
        return None

    def _resolve_model_key(
        self, name: str, model_state: Union[dict, OrderedDict]
    ) -> Optional[str]:
        """
        Resolve a compressed key to a compatible key in model_state.
        """
        candidates = [name]
        if name.startswith("module."):
            candidates.append(name[len("module.") :])
