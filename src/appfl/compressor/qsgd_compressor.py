...
class QSGDCompressor(BaseCompressor):
    ...
    def _decompress_model(
        ...
        template_state = model.state_dict() if hasattr(model, "state_dict") else {}
        decompressed_weights = OrderedDict()
        for name, cmp_bytes in compressed_weights.items():
            ref_name = self._resolve_template_key(name, template_state)
            ref_param = template_state.get(ref_name, None)

            if self._looks_like_qsgd_payload(cmp_bytes):
                numel = (
                    int(ref_param.numel())
                    if ref_param is not None
                    else int(pickle.loads(cmp_bytes).get("numel", 0))
                )
                ...
                arr = self._lossless_decompress(cmp_bytes)

            if ref_param is not None:
                if ref_param.shape == torch.Size([]):
                    copy_arr = deepcopy(arr)
                    copy_tensor = torch.from_numpy(copy_arr)
                    if copy_tensor.numel() == 1:
                        tensor = copy_tensor.reshape(()).to(ref_param.dtype)
                    else:
                        # Fallback for malformed scalar payloads.
                        tensor = copy_tensor.reshape(-1)[0].to(ref_param.dtype)
                else:
                    copy_arr = deepcopy(arr)
                    copy_tensor = torch.from_numpy(copy_arr).to(ref_param.dtype)
                    tensor = copy_tensor.reshape(ref_param.shape)
            else:
                # Fallback when template key cannot be resolved.
                tensor = torch.from_numpy(arr)

            # Keep the original key name sent by the client.
            decompressed_weights[name] = tensor
        return decompressed_weights

    @staticmethod
    def _looks_like_qsgd_payload(cmp_data: bytes) -> bool:
        try:
            payload = pickle.loads(cmp_data)
            return (
                isinstance(payload, dict)
                and "levels" in payload
                and "signs" in payload
                and "numel" in payload
            )
        except Exception:
            return False
    ...
    def _resolve_template_key(
        key: str, template_state: Union[dict, OrderedDict]
    ) -> str:
        ...
        replacements = [
            ...
            (".norm_layer.", ".block.1."),
            (".block.1.", ".norm_layer."),
            (".activation_layer.", ".block.2."),
            (".block.2.", ".activation_layer."),
            (".dropout_layer.", ".block.3."),
            (".block.3.", ".dropout_layer."),
        ]
    ...
    def _lossless_decompress(self, data: bytes) -> np.ndarray:
        if self.lossless_compressor == "zstd":
            ...
            raise NotImplementedError
        # New format: pickled numpy array to preserve dtype and shape.
        try:
            ...
            pass
        # Backward compatibility fallback for older raw-byte payloads.
