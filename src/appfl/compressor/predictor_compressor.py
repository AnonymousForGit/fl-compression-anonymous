...
class PredictorCompressor(BaseCompressor):
    ...
    def __init__(self, compressor_config: DictConfig):
        ...
        # Logging
        self.logger = logging.getLogger(f"{__name__}.PredictorCompressor")
        if not self.logger.handlers:
            # Use a repository-relative log path.
            repo_root = pathlib.Path(__file__).resolve().parents[3]
            log_file = repo_root / "examples" / "output" / "predictor_compressor.log"
        ...
        # self.logger.info(f"Initializing PredictorCompressor with momentum_lr={self.momentum_lr}, "
        ...
        repo_root = pathlib.Path(__file__).resolve().parents[3]
        configured_path = compressor_config.get("sz_lib_path", None)
        if configured_path is not None:
            self.compressor_lib_path = str(pathlib.Path(str(configured_path)))
        else:
            base = repo_root / "SZ_NP"
            candidates = [
                base / "lib64" / f"libSZ3c{ext}",
                base / "lib" / f"libSZ3c{ext}",
                base / "build" / "lib64" / f"libSZ3c{ext}",
                base / "build" / "lib" / f"libSZ3c{ext}",
            ]
            if base.exists():
                candidates.extend(sorted(base.rglob(f"libSZ3c{ext}")))
            existing = next((p for p in candidates if p.exists()), None)
            self.compressor_lib_path = str(
                existing if existing is not None else candidates[0]
            )
    ...
    def _is_supported_conv_weight(self, arr: np.ndarray) -> bool:
        return arr.ndim in (4, 5)

    def _kernel_axes(self, arr: np.ndarray) -> Tuple[int, ...]:
        if not self._is_supported_conv_weight(arr):
            raise ValueError(f"Unsupported convolution weight ndim={arr.ndim}, expected 4 or 5")
        return tuple(range(2, arr.ndim))
    ...
    def _create_compressed_data(self, gradient: np.ndarray, client_id: str, layer_name: str) -> Dict[str, Any]:
        ...
        if self._is_supported_conv_weight(current_grad) and self._should_lossy(layer_name, current_grad):
    ...
    def _simulate_reconstruction(self, compressed_result: dict, original_grad: np.ndarray,
        ...
        if compressed_result["type"] == "momentum_predicted":
            ...
            if num_pred > 0:
                ...
                sign_view_shape = (num_pred,) + (1,) * kernel_ndim
                predicted_kernel = dominant_sign_arr.reshape(sign_view_shape) * abs_pred
    ...
    def _compress_conv_layer(self, current_grad: np.ndarray, prev_grad: np.ndarray,
        ...
        if not self._is_supported_conv_weight(current_grad):
            raise ValueError(
                f"_compress_conv_layer expects Conv2d/Conv3d weights, got shape={current_grad.shape}"
            )
        out_ch, in_ch = current_grad.shape[0], current_grad.shape[1]
        kernel_axes = self._kernel_axes(current_grad)
        kernel_size = int(np.prod(current_grad.shape[2:]))
        kernel_ndim = current_grad.ndim - 2
        ...
        positives = np.sum(kernel_signs_full > 0, axis=kernel_axes)
        negatives = np.sum(kernel_signs_full < 0, axis=kernel_axes)
        zeros = np.sum(kernel_signs_full == 0, axis=kernel_axes)
        ...
        avg_sign_consistency = float(np.mean(consistencies)) if consistencies.size > 0 else 0.0
        min_sign_consistency = float(np.min(consistencies)) if consistencies.size > 0 else 0.0
        max_sign_consistency = float(np.max(consistencies)) if consistencies.size > 0 else 0.0
        ...
        if predicted_kernels > 0:
            ...
            sign_view_shape = (predicted_kernels,) + (1,) * kernel_ndim
            predicted_kernel = dominant_sign_sel.reshape(sign_view_shape) * abs_pred
        ...
        original_norm = float(np.linalg.norm(current_grad))
        residual_norm = float(np.linalg.norm(residual_data))
        residual_norm_ratio = residual_norm / max(original_norm, 1e-12)
        ...
        return {
            ...
            "max_sign_consistency": max_sign_consistency,
    ...
    def compress_model(self,
        ...
        sign_consistency_values: List[float] = []
        residual_norm_ratios: List[float] = []
        layer_count = 0

        for layer_name, layer_params in model_params.items():
            if (
                isinstance(layer_params, torch.Tensor)
                and layer_params.ndim in (4, 5)
                and self._should_lossy(layer_name, layer_params.detach().cpu().numpy())
            ):
                ...
                if "avg_sign_consistency" in compressed_layer:
                    sign_consistency_values.append(compressed_layer["avg_sign_consistency"])
                if "residual_norm_ratio" in compressed_layer:
                    residual_norm_ratios.append(compressed_layer["residual_norm_ratio"])
        ...
        if layer_count > 0:
            ...
            if sign_consistency_values:
                avg_sign_consistency = float(np.mean(sign_consistency_values))
                min_sign_consistency = float(np.min(sign_consistency_values))
                max_sign_consistency = float(np.max(sign_consistency_values))
                self.compression_stats["sign_consistency_means"].append(avg_sign_consistency)
                avg_residual_norm_ratio = (
                    float(np.mean(residual_norm_ratios)) if residual_norm_ratios else float("nan")
                )
                if residual_norm_ratios:
                    self.compression_stats["residual_norm_ratios"].append(avg_residual_norm_ratio)
                self.logger.info(
                    "PredictorCompressor conv stats (client=%s): sign_consistency(avg/min/max)=%.4f/%.4f/%.4f, "
                    "prediction_ratio=%.4f, residual_norm_ratio=%.4f, conv_layers=%d",
                    client_id,
                    avg_sign_consistency,
                    min_sign_consistency,
                    max_sign_consistency,
                    avg_prediction_ratio,
                    avg_residual_norm_ratio,
                    len(sign_consistency_values),
                )
        ...
        for layer_name, compressed_layer in compressed_layers.items():
            ...
            try:
                if layer_type == "direct":
                    ...
                    if shape and isinstance(shape, (list, tuple)) and len(shape) in (4, 5):
                        layer_key = _conv_layer_key(shape)
    ...
    def get_compression_stats(self) -> Dict[str, Any]:
        ...
        if stats["sign_consistency_means"]:
            stats["avg_sign_consistency"] = float(np.mean(stats["sign_consistency_means"]))
            stats["std_sign_consistency"] = float(np.std(stats["sign_consistency_means"]))
        if stats["residual_norm_ratios"]:
            stats["avg_residual_norm_ratio"] = float(np.mean(stats["residual_norm_ratios"]))
            stats["std_residual_norm_ratio"] = float(np.std(stats["residual_norm_ratios"]))
