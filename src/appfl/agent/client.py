...
class ClientAgent:
    ...
    def get_parameters(
        ...
        # Record original size before compression (in bytes)
        original_size = 0
        for param in params.values():
            if torch.is_tensor(param):
                original_size += int(param.numel() * param.element_size())
            elif isinstance(param, np.ndarray):
                original_size += int(param.nbytes)
        ...
        if self.enable_compression:
            ...
            ratio_str = (
                ...
                if np.isfinite(compression_ratio)
                else "inf"
            )
            self.logger.info(
                f"Compression stats: original={original_size}B compressed={compressed_size}B ratio={ratio_str}"
            )
            if isinstance(metadata, dict):
                metadata["compression_stats"] = self.compression_stats
            if self.optimize_memory:
                gc.collect()
    ...
    def _load_data(self) -> None:
        ...
        if hasattr(self.client_agent_config.data_configs, "dataset_source"):
            ...
            dataset_pair = run_function_from_file(
            ...
            if dataset_pair is None:
                raise RuntimeError(
                    "Failed to load dataset. Please check `data_configs.dataset_path`, "
                    "`data_configs.dataset_name`, and `data_configs.dataset_kwargs`."
                )
            self.train_dataset, self.val_dataset = dataset_pair
